...
import TransactionSearchAndExport from './TransactionSearchAndExport'; // Corrected path

// Import selectors and the fetch action from your account slice
import {
  fetchBankAccounts,
...
const ReportsPage = () => {
  ...
  const [exportMessage, setExportMessage] = useState(null); // State for export messages
  ...
  const handleGenerateStatement = useCallback(async (e) => {
    ...
    setLoading(true);
    setError(null);
    setExportMessage(null); // Clear any previous export messages
    setStatementData([]);
    ...
    try {
      ...
      console.log("Statement data fetched:", response.data); // DEBUG: Check fetched data
  ...
  const handleExportCsv = () => {
    setExportMessage(null); // Clear previous export messages
    console.log("Export CSV button clicked."); // DEBUG

    if (!statementData.length) {
      setExportMessage({ type: 'error', text: 'No data to export for the statement. Please generate a statement first.' }); // Replaced alert
      console.log("No statement data to export."); // DEBUG
      return;
    }

    console.log("Statement data available for export:", statementData); // DEBUG

    const headers = [
      "Sr. No.", "Date", "Transaction Type", "Amount", "Description",
      "Transaction Head", "Transaction Mode", "Cheque No.", "Is P/P Cheque", "Is Reconciled", "Created By" // Added Is P/P Cheque, Is Reconciled
    ];
    const rows = statementData.map((tx, index) => {
      // DEBUG: Check individual transaction properties
      console.log(`Processing transaction ${tx.id}: is_pp=${tx.is_pp}, is_reconciled=${tx.is_reconciled}`);
      return [
        index + 1,
        ...
        tx.transaction_mode,
        tx.cheque_no || '-',
        tx.is_pp ? 'Yes' : 'No', // Include is_pp
        tx.is_reconciled ? 'Yes' : 'No', // Include is_reconciled
    ...
    console.log("Generated CSV Content (first 200 chars):", csvContent.substring(0, 200)); // DEBUG

    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a');

    console.log("link.download support:", link.download !== undefined); // DEBUG

    if (link.download !== undefined) {
      const url = URL.createObjectURL(blob);
      link.setAttribute('href', url);
      link.setAttribute('download', `statement_${accountDetails?.account_number || 'unknown'}_${startDate}_to_${endDate}.csv`);
      link.style.visibility = 'hidden';
      document.body.appendChild(link);
      link.click(); // Programmatically click the link to trigger download
      document.body.removeChild(link); // Clean up the link element
      URL.revokeObjectURL(url); // Clean up the URL object to free memory
      setExportMessage({ type: 'success', text: 'CSV exported successfully!' }); // Success message
      console.log("CSV download initiated successfully."); // DEBUG
    } else {
      setExportMessage({ type: 'error', text: 'Your browser does not support automatic CSV download. Please try a different browser.' }); // Error message
      console.log("Browser does not support automatic CSV download."); // DEBUG
    }
  };
  ...
  return (
    <div className="container mx-auto p-4 md:p-6">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-8">
        ...
        {statementData.length > 0 && accountDetails && (
          <div className="mt-8">
            ...
            {exportMessage && ( // Display export message
            ...
            <div className="flex justify-end gap-4 mb-4">
              <button
                ...
                disabled={loading || statementData.length === 0} // Added disabled prop
              >
                Export CSV
              </button>
            </div>
      ...
      <div className="mt-8">
        {/* Pass selectedAccountId to TransactionSearchAndExport */}
