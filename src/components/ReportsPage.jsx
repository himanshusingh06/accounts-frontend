...
const ReportsPage = () => {
  ...
  const [statementData, setStatementData] = useState([]); // Transactions for the statement
  const [accountDetails, setAccountDetails] = useState(null); // Details of the selected account
    ...
    const fetchAccounts = async () => {
      try {
        ...
        if (response.data.length > 0) {
          setSelectedAccountId(response.data[0].id); // Select the first account by default
        }
      } catch (err) {
        console.error('Error fetching bank accounts:', err);
        setError('Failed to load bank accounts for statement generation.');
      }
    };
    fetchAccounts();
  }, []);
  ...
  return (
    <div className="container mx-auto p-4 md:p-6">
      <h1 className="text-4xl font-bold text-blue-800 mb-6" data-aos="fade-down">Reports & Statements</h1>
